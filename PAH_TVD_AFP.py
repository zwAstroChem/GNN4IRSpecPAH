...
# -----------------------
# Total Variation Distance Loss (for training)
# -----------------------
def tv_loss(pred, target):
    """
    Total Variation Distance loss function
    Formula: TV(P,Q) = 0.5 * Σ|p_i - q_i|
    ...
    absolute_diff = torch.abs(p - q)
    total_variation = 0.5 * torch.sum(absolute_diff, dim=1)
...
def train_one_epoch(model, loader, optimizer):
    ...
    for batch in loader:
        ...
        optimizer.zero_grad()
        preds = model(batch)
        loss = tv_loss(preds, batch.y)  # Training uses TV loss
        loss.backward()
        optimizer.step()
...
for fold_idx in range(5):
    ...
    for epoch in range(1, MAX_EPOCHS + 1):
        ...
        print(f"Epoch {epoch:03d} | Train TV Loss: {train_loss:.4f} | Val EMD: {val_loss:.4f}")
    ...
    # -----------------------
    # Test set EMD mean ± std for each fold
    # -----------------------
    model.eval()
    emd_list = []
    with torch.no_grad():
    ...
    print(f"ALL_High_TVD_Fold {fold_idx + 1} Test EMD: {avg_emd:.5f} ± {std_emd:.5f}")
    ...
    fold_path = os.path.join(output_dir, f"ALL_High_TVD_Fold_{fold_idx + 1}_results.pickle")
    with open(fold_path, 'wb') as f:
        pickle.dump(results, f)
    print(f"ALL_High_TVD_Fold {fold_idx + 1} results saved to {fold_path}")
