...
def sis_loss(pred, target):
    ...
    # SID
    sid = torch.sum(p * torch.log(p / q), dim=1) + torch.sum(q * torch.log(q / p), dim=1)
...
for fold_idx in range(5):
    ...
    for epoch in range(1, MAX_EPOCHS + 1):
        ...
        print(f"Epoch {epoch:03d} | Train SIS Loss: {train_loss:.4f} | Val EMD: {val_loss:.4f}")
    ...
    fold_path = os.path.join(output_dir, f"ALL_High_SIS_Fold_{fold_idx + 1}_results.pickle")
    with open(fold_path, 'wb') as f:
        pickle.dump(results, f)
    print(f"ALL_High_SIS_Fold {fold_idx + 1} results saved to {fold_path}")
