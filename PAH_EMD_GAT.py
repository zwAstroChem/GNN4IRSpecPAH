...
# -----------------------
class GAT_Spectra_Model(nn.Module):
    def __init__(self, in_channels, hidden_dim, out_dim, num_layers=3, heads=4, dropout=0.1):
        super(GAT_Spectra_Model, self).__init__()
        ...
        self.out_lin = nn.Linear(hidden_dim * heads, out_dim)
...
for fold_idx in range(5):
    ...
    model = GAT_Spectra_Model(
        in_channels=node_feat_dim,
        hidden_dim=64,     # GAT models usually use smaller hidden dimensions
    ...
    print(f"Low_GAT_Fold {fold_idx + 1} Test EMD: {avg_emd:.5f} ± {std_emd:.5f}")
    ...
    fold_path = os.path.join(output_dir, f"Low_GAT_Fold_{fold_idx + 1}_results.pickle")
    with open(fold_path, 'wb') as f:
        pickle.dump(results, f)
    print(f"Low_GAT_Fold {fold_idx + 1} results saved to {fold_path}")
