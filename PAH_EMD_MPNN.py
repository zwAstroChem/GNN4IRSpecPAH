...
# -----------------------
# SMILES → PyG Data (including smiles)
# -----------------------
...
# -----------------------
# MPNN model definition (NNConv)
# -----------------------
class MPNN_Spectra_Model(nn.Module):
    def __init__(self, in_channels, hidden_dim, out_dim, edge_dim, num_layers=3, dropout=0.1):
        super(MPNN_Spectra_Model, self).__init__()
        ...
        self.convs.append(NNConv(in_channels, hidden_dim, nn_edge, aggr='mean'))
...
for fold_idx in range(5):
    ...
    print(f"Low_MPNN_Fold {fold_idx + 1} Test EMD: {avg_emd:.5f} ± {std_emd:.5f}")
    ...
    fold_path = os.path.join(output_dir, f"Low_MPNN_Fold_{fold_idx + 1}_results.pickle")
    with open(fold_path, 'wb') as f:
        pickle.dump(results, f)
    print(f"Low_MPNN_Fold {fold_idx + 1} results saved to {fold_path}")
