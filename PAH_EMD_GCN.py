...
class GCN_Spectra_Model(nn.Module):
    def __init__(self, in_channels, hidden_dim, out_dim, num_layers=3, dropout=0.1):
        super(GCN_Spectra_Model, self).__init__()
...
for fold_idx in range(5):
    ...
    fold_path = os.path.join(output_dir, f"Low_GCN_Fold_{fold_idx + 1}_results.pickle")
    with open(fold_path, 'wb') as f:
        pickle.dump(results, f)
    print(f"Low_GCN_Fold {fold_idx + 1} results saved to {fold_path}")
